...
#
# Reads thin_dump XML for slice0 and slice1, walks the union of every
...
#
# A set bit on the ld0 (dn0) bitmap for (slice, grp) is also set on the ld1 (dn1)
# bitmap for the same (slice, grp): the grp's thindata lives on that grp's raid1
# mirror, so both legs hold the same block.
#
...
# (slice, grp) pair both ld0 and ld1 get a bitmap (raid1 mirror). Filename
# follows the -real LV identity: dnv-<dn>-sp0-slice<slice>-grp<grp>-ld<ld>.bit
LD_TARGETS = [
    (dn, slice, grp, ld)
...
def main(argv):
    ...
    # bitmaps[(dn, slice, grp, ld)] -> list of 125 '0'/'1'
    bitmaps = {}
    for dn, slice, grp, ld in LD_TARGETS:
        bitmaps[(dn, slice, grp, ld)] = empty_bitmap()

    total_allocated = 0
    for slice in (0, 1):
        xml_path = os.path.join(work_dir, "thin_dump_slice%d.xml" % slice)
        allocated = parse_allocations(xml_path)
        total_allocated += len(allocated)
        if not allocated:
            sys.stderr.write("[WARN] slice%d: no allocated pool blocks in %s\n"
                             % (slice, xml_path))
            continue
        for n in sorted(allocated):
            res = block_to_ld_bit(n)
            if res is None:
                sys.stderr.write("[WARN] slice%d: pool block %d out of range, "
                                 "skipping\n" % (slice, n))
                continue
            grp, ld_bit = res
            if ld_bit >= LD_BITS:
                sys.stderr.write("[WARN] slice%d: block %d -> ld_bit %d out of "
                                 "range, skipping\n" % (slice, n, ld_bit))
                continue
            # raid1 mirror: set the bit on both ld0 (dn0) and ld1 (dn1) for
            # this (slice, grp).
            bitmaps[("dn0", slice, grp, "ld0")][ld_bit] = "1"
            bitmaps[("dn1", slice, grp, "ld1")][ld_bit] = "1"

    if total_allocated == 0:
        sys.stderr.write("[WARN] empty pool: all 8 bitmaps are zero\n")

    for dn, slice, grp, ld in LD_TARGETS:
        name = "dnv-%s-sp0-slice%d-grp%d-%s" % (dn, slice, grp, ld)
        path = os.path.join(work_dir, name + ".bit")
        with open(path, "w") as f:
            f.write(to_file(bitmaps[(dn, slice, grp, ld)]))
