...
Create `check.sh` + `poc/vd_bitmap.py` under `poc/` that, after `setup.sh all`
+ a short `host0_io.sh` run, audits the thin-pool's allocated data blocks
against what was physically written to the VDs on cn0. The audit dumps each
leg's thin-pool metadata with `thin_dump`, derives an 8-VD bitmap (one bit per
4 MiB VD block), then on cn0 reads each marked 4 MiB block directly from the
VDs and confirms it is non-zero (i.e. actually written, not thin-pool
...
| 3 | Number of bitmaps | 8, one per `-real` VD (`dnv-<dn>-da0-leg<leg>-grp<grp>-vd<vd>-real`): 4 on dn0 (vd0), 4 on dn1 (vd1) |
...
| 7 | Bitmap coverage | Option A: full 125-block VD bitmap (500 MiB / 4 MiB = 125). Bits 0..2 always 0 (raid1-meta + 8 MiB thinmeta), bits 3..124 reflect thindata allocation |
...
| 9 | Bitmap filenames | `dnv-<dn>-da0-leg<leg>-grp<grp>-vd<vd>.bit` (matches `-real` LV identity) |
...
| 16 | XML parsing | Union of `[data_begin, data_end)` ranges across ALL `<device>` elements (not just dev_id=0). Handles future snaps; semantically a data block is "written" if any thin device references it |
| 17 | Empty-pool handling | Defensive: empty XML / no mappings -> all-zero bitmaps + warning, not a crash |
| 18 | Block->bit formula | For leg L, allocated pool data block `N`: `grp = N // 122`, `blk = N % 122` (488 MiB / 4 MiB = 122), set bit `3 + blk` on both `vd0` (dn0) and `vd1` (dn1) for that `(leg,grp)` (raid1 mirror = same blocks on both sides) |
...
| 21 | Phase 3 SSH style | Minimal heredoc (no `_emit_common`); bake VD device paths + bitmap dir in as Python literals |
| 22 | Verifier output | JSONL: one record per VD `{vd, set, verified, zero, status}` + a final human summary line. Matches `host0_io.sh`'s JSONL convention |
...
| 24 | Exit code | `check.sh` exit 0 only if all 8 VDs report `ok` or `empty`; non-zero on any failure (precondition missing, thin_dump error, XML parse error, missing device, read error, zero mismatch) |
...
| 27 | vd_bitmap.py interface | One positional arg (the work dir), defaulting to `/tmp/dnv-check/` |
...
| `SEC_SNAP` (thin virt) | 2097152 | 1024 | 256 |
...
### 3.4 `vd_bitmap.py` -- new script (Phase 2, step 9)

Standalone, stdlib-only (`xml.etree.ElementTree`). Interface:

```
python3 poc/vd_bitmap.py [<work_dir>]
...
```
dnv-dn0-da0-leg0-grp0-vd0.bit
dnv-dn0-da0-leg0-grp1-vd0.bit
dnv-dn0-da0-leg1-grp0-vd0.bit
dnv-dn0-da0-leg1-grp1-vd0.bit
dnv-dn1-da0-leg0-grp0-vd1.bit
dnv-dn1-da0-leg0-grp1-vd1.bit
dnv-dn1-da0-leg1-grp0-vd1.bit
dnv-dn1-da0-leg1-grp1-vd1.bit
```

Each file: 125 ASCII '0'/'1' chars + trailing `\n` (126 bytes). Bit 0 =
leftmost (VD offset `[0,4MiB)` = raid1-meta), bit 124 = rightmost (offset
...
for leg in {0,1}:
    ...
    for N in allocated:
        ...
        # raid1 mirror: same block on both sides (vd0 on dn0, vd1 on dn1)
        bitmap[dn0][leg][grp][vd0].set(vd_bit)
        bitmap[dn1][leg][grp][vd1].set(vd_bit)
```
...
Precondition: `dm_exists dnv-cn0-da0-leg{0,1}-thinpool` on cn0 via SSH,
else `_fail "run setup.sh all first"`.

Per leg (0,1): minimal direct SSH (no `_emit_common` heredoc):
```
sudo dmsetup suspend dnv-cn0-da0-leg${leg}-thinpool 2>/dev/null
sudo thin_dump /dev/mapper/dnv-cn0-da0-leg${leg}-thinmeta
sudo dmsetup resume dnv-cn0-da0-leg${leg}-thinpool 2>/dev/null
```
...
**Phase 2 -- derive 8 VD bitmaps (step 9):**

```
_t "vd_bitmap.py" python3 "$(dirname "$0")/vd_bitmap.py" /tmp/dnv-check/
```
...
verifier heredoc on cn0 (no `_emit_common`); bake the 8 VD device paths
(`/dev/mapper/dnv-<dn>-da0-leg<leg>-grp<grp>-vd<vd>-cn0`) and the bitmap dir
in as Python literals. Run it, capture JSONL output. Wrap with
`_t "verify VDs on cn0"`. `slow_summary` at phase end.

Verifier logic (per VD):
1. Read the 125-char bitstring from `/tmp/dnv-check/dnv-<dn>-...-vd<vd>.bit`.
2. Open `/dev/mapper/dnv-<dn>-da0-leg<leg>-grp<grp>-vd<vd>-cn0` with `O_RDWR|O_DIRECT` (or `O_RDONLY`).
...
5. Emit one JSON line: `{"vd":"dnv-...","set":N,"verified":M,"zero":K,"status":"ok|empty|missing_device|read_error|fail"}`.
...
- Prints each VD's JSON line.
- Prints a final summary line: `[OK] N/8 VDs verified, 0 zero-block mismatches` or `[FAIL] M VDs failed: <list>`.
...
Within one VD (`dnv-<dn>-da0-leg<leg>-grp<grp>-vd<vd>-real`, 500 MiB):
...
- IDs 0..121 -> grp0-thindata -> VD bit `3 + ID`.
- IDs 122..243 -> grp1-thindata -> VD bit `3 + (ID - 122)`.

Each grp's thindata lives on that grp's raid1, mirrored across vd0 (dn0) and
vd1 (dn1). So a set bit on the `vd0` bitmap for `(leg,grp)` is also set on the
`vd1` bitmap for the same `(leg,grp)`.
...
grp1's VD bitmaps are nearly all-zero except the anchor block. grp0's VD
bitmaps have ~8-16 set bits.

Total verification work: ~9-17 set bits per non-empty VD x 8 VDs = ~72-136
...
Expected `check.sh` output: exit 0, ~4 VDs with `ok` (grp0 VDs on dn0+dn1 for
both legs, each with ~8-16 set blocks), ~4 VDs with `empty` or near-empty
(grp1 VDs), 0 zero-block mismatches.
