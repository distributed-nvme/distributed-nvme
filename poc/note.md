...
A six-VM testbed that builds a **storage pool** (`sp`) out of commodity parts —
...
| term | meaning |
| --- | --- |
| **dn** | **Disk node.** A Linux server that holds physical disks (loop files) and exports logical-disk slices over NVMe-oF/TCP to the controller nodes. There are two: `dn0`, `dn1`. |
| **cn** | **Controller node.** A Linux server that imports logical disks from the DNs, assembles them into a raid1 + thin-pool + raid0 stack, and re-exports the result to the host. There are two: `cn0` (active), `cn1` (standby). A `cn` hosts one **cntlr** (the virtual controller of a `sp`) — see §2.3. |
...
```
            ...
            cn0 (active)  cn1 (standby)
              |             |
              | NVMe-oF/TCP (per-LD NQN, hostnqn-scoped)
              |             |
         +----+----+   +----+----+
         |         |   |         |
        dn0       dn1  dn0      dn1
...
Each LD on a DN has a symmetric fault-injection pair for both CNs:

```
  -real  → LVM LV in dnv-<dn>-sp0-vg (loop is the PV; the actual data)
...
`nvme_conn`, `nvme_disc`, `nvme_disconnect_all_dnv_ld`, `ana_set`, `stas_install`,
`stas_write_config`, `stas_start`, `stas_stop_restore`, `rd_ok`, `rd_eio`, `rd_hang`,
`verify_summary`, `_t`, `slow_summary`.

`nvmet_remove_all_dnv_subsys` and `nvme_disconnect_all_dnv_ld` are
topology-agnostic teardown safety nets: they scan `nvmet/subsystems/` and
`/sys/class/nvme/*/subsysnqn` for `:dn:*` NQNs (the LD subsystem/connections)
...
| 5 | dn1 | Same swap. |
| 6 | cn1 | `create_cntlr_active` rebuilds the stack (raid1 + thin-pool + td0). The thin-pool metadata is cn0's committed copy — `create_thin 0` fails (inherited), which is the **td-0 inheritance invariant**. Build raid0, repoint exp → real, resume. Queued host I/O drains. |
...
2. **stafd** on host0 is configured with exactly one discovery controller
   ...
   cn0 and cn1.
3. **stacd** sees the CN discovery services advertising `nqn...:sp:sp0:td0:exp0`
...
failover            exit 0   8.8 s wall, td-0 inherited from cn0
...
* **LVM for `-real`**: the DN-side `-real` device (the one backed directly by
  ...
  (data integrity, td-0 inheritance, failover) hold regardless of where LVM
