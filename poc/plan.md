...
renamed terminology, a td/exp abstraction, and a flat `common.sh` API. Then test
...
| `ld` | logical disk, dn->cn via nvmeof | `ld0`/`ld1` + `-cn0`/`-cn1` exports |
| `grp` | thin-pool underlying group (raid1 slice pair) | `grp0`/`grp1` (unchanged) |
| `leg` | raid0 underlying disk | `stripe0`/`stripe1` |
| `side` | raid1 underlying ld | `ld0`/`ld1` (raid1 "leg" comments) |
| `sp` | storage pool (container, N legs, thin pools, tds) | `vol0` |
| `td` | thin device id in a sp (shared across all legs) | hardcoded `thin 23` |
| `exp` | exporter (raid0 of td thin-devs + nvmeof export) | `${vp}` exported volume |

Node roles:
- `dn`  = disk node (linux server with physical disks)
- `cn`  = controller node (linux server exporting nvmet targets to nvme hosts)

**Renames:** `vol`->`sp` everywhere (NQN, device names); `stripe`->`leg`; raid1 "leg"
comments->"side". `RAID0_CHUNK_SECTORS` stays (it is a chunk size, not a leg).

## 3. Resource naming
- Host NQN: `nqn.2026-07.org.dnv:sp:sp0:td0:exp0`
- DN linear/ld: `dnv-<dn>-<sp>-leg<leg>-grp<grp>-ld<ld>-cn<cn>` (+ `-real`,
  `-err-<cn>`, `-delay-<cn>`)
- CN raid1: `dnv-<cn>-<sp>-leg<leg>-grp<grp>-raid1-side<side>`
  (+ `-meta-side0`/`-data-side0`/...)
- CN thin pool: `dnv-<cn>-<sp>-leg<leg>-thinpool`
  (+ `-thinmeta`/`-thindata`/`-grp<grp>-thinmeta`/`-grp<grp>-thindata`)
- CN exp: `dnv-<cn>-<sp>-td<id>-exp<id>` (+ `-real`/`-error`/`-delay`)
...
create_ld    <dn_name> <dn_ip> <cn_name> <cn_ip> <ld_id>   # full symmetric fault-injection pair + nvmet export
delete_ld    <dn_name> <dn_ip> <cn_name> <cn_ip> <ld_id>
connect_ld    <cn_name> <cn_ip> <dn_name> <dn_ip> <ld_id> <hostnqn> <hostid>
disconnect_ld <cn_name> <cn_ip> <dn_name> <dn_ip> <ld_id>
create_grp   <cn_name> <cn_ip> <sp> <leg> <grp> <hostnqn> <hostid>   # connects lds internally + builds raid1 + thinmeta/thindata
delete_grp   <cn_name> <cn_ip> <sp> <leg> <grp>
create_leg   <cn_name> <cn_ip> <sp> <leg>                           # concat grps -> thinpool -> create_td 0 0
delete_leg   <cn_name> <cn_ip> <sp> <leg>
create_sp_active   <cn_name> <cn_ip> <sp> <nlegs> <hostnqn> <hostid>   # calls create_grp+create_leg for all legs
create_sp_standby  <cn_name> <cn_ip> <sp> <nlegs> <hostnqn> <hostid>   # calls connect_ld for all the sp's lds (no stack)
delete_sp_active   <cn_name> <cn_ip> <sp>
delete_sp_standby  <cn_name> <cn_ip> <sp>
create_td  <cn_name> <cn_ip> <sp> <new_id> <src_id>   # create_thin (id 0,src 0) OR create_snap (src>0) across ALL leg pools
delete_td  <cn_name> <cn_ip> <sp> <id>
create_exp_active  <cn_name> <cn_ip> <sp> <td_id> <host_nqn> <host_id> <cntlid_min> <cntlid_max>   # raid0+real+error+delay+exp, ANA optimized
create_exp_standby <cn_name> <cn_ip> <sp> <td_id> <host_nqn> <host_id> <cntlid_min> <cntlid_max>   # error+delay+exp stub, ANA inaccessible
delete_exp_active  <cn_name> <cn_ip> <sp> <td_id>
delete_exp_standby <cn_name> <cn_ip> <sp> <td_id>
```
...
- `set_host_identity`
- `disarm_ld_delay`/`arm_ld_delay` (dn-side delay-cn1 disarm/arm)
...
- dn0/dn1: `create_pd` -> `create_ld` (x4 lds per dn: leg0/1 x grp0/1, each for cn0 and cn1)
- cn0: `create_grp` x4 -> `create_sp_active cn0 sp0 2 <hnqn> <hid>`
  -> `create_exp_active cn0 sp0 0 host0... 1 255`
- cn1 namespace-scan coordination: `disarm_ld_delay` dn0/dn1
  -> `create_sp_standby cn1 sp0 2 <hnqn> <hid>` -> `arm_ld_delay` dn0/dn1
- cn1: `create_exp_standby cn1 sp0 0 host0... 256 511`
...
`stas_stop_restore` -> cn1 -> cn0 (via `delete_exp_*` -> `delete_sp_*` ->
`delete_grp`/`delete_leg` -> `disconnect_ld`) -> dn0/dn1 (`delete_ld` -> `delete_pd`)
...
`common.sh`'s names/tables/sizes. Uses `create_sp_active` for cn1's step-6 rebuild.
...
3. cn0: ANA->inaccessible, flushing-suspend, repoint exp->delay early, dismantle
   stack (commits td-0 metadata)
4/5. dn0/dn1: swap `-cn0`->delay, `-cn1`->real
6. cn1: `create_sp_active` (rebuild, `create_td cn1 sp0 0 0` should fail =>
   inherited), repoint exp->real, resume
7. cn0: exp->delay

- `force` mode: skip 3 and 7. td-0 inheritance invariant preserved.

## 8. `host0_io.sh` + `servers.txt`
- `host0_io.sh`: copy t03 verbatim, change only `NQN_VOL` to
  `nqn.2026-07.org.dnv:sp:sp0:td0:exp0`.
...
1. **td/exp model** -- `create_exp` takes a td/dev id; setup always passes 0.
   td 0 is the live writable origin.
2. **raid1 side naming** -- use "side" (mirror conflicts with future raid5/6).
3. **sp scope** -- one sp is the container for the whole volume. A sp declares N legs
   (2 in t03). A td is a set of N thin-device ids (one per leg's pool). An exp
   creates N thin devices (one per leg's td id) + a raid0 of them, and exports
   that raid0 to a host.
4. **td id coordination** -- shared td id across all legs. `create_td` messages
...
6. **resource naming** -- `sp` replaces `vol` everywhere; leg/side/grp/-cn retained.
   Host NQN: `...:sp:sp0:td0:exp0`.
7. **ld fault-injection pair** -- symmetric: both cns get full `-real`/`-err`/`-delay`/`-cn`
...
8. **create_grp layering** -- `create_grp` owns the raid1 + thinmeta/thindata slices
   (one grp = one mirrored slice pair). `create_sp` assembles grps into a per-leg
   thin pool.
9. **create_leg vs create_sp** -- `create_sp` orchestrates (calls `create_leg` per
   leg); `create_leg` assembles its grps into a thin pool + creates default td 0;
   `create_grp` is the leaf.
10. **create_exp** -- builds the full stack: per-leg thin devices from the td id ->
    raid0 (`-real`) -> `-error`/`-delay`/`-exp` (exported linear) -> nvmet export to
    host, with ANA state + cntlid inferred from the cn's role.
11. **td primitives** -- one `create_td` function handles both `create_thin`
    (default empty origin, id 0) and `create_snap` (derived). `delete_td` issues
    `delete <id>` to all pools.
12. **connect_ld/disconnect_ld** -- added to the API. `create_grp` calls `connect_ld`
    internally for both cns; on cn1 the caller still invokes `connect_ld`. The
    disarm/arm coordination for cn1's namespace-scan hangs lives in setup.sh around
    the connect, not in `common.sh`.
13. **active/standby split** -- `create_sp_active`/`create_sp_standby` and
    `create_exp_active`/`create_exp_standby` (+ deletes). `create_sp_active` calls
    `create_grp` (builds the stack); `create_sp_standby` calls `connect_ld` (holds
    ...
    `create_sp_active` + repointing the exp.
14. **failover sequence** -- 14a option 2: failover keeps t03's inline-dmsetup style,
    ...
    names/tables/sizes. 14b option 1: td-0 inheritance invariant preserved
    (`create_td cn1 sp0 0 0` should fail => metadata inherited).
...
16. **signatures** -- 16a: pass `hostnqn`/`hostid` on every cn-side call. 16b: pass
    ...
    active/standby. 16c: `ld_id`/`td_id` are plain integers.
17. **setup topology** -- full t03 topology carries over: 6 nodes, ref0 anchor trick,
    ...
    coordination. Only resource names and td/exp layering change.
...
20. **teardown/failover inline vs API** -- asymmetric: teardown uses `common.sh`'s
    ...
    calls `create_sp_active` for step 6's rebuild.
