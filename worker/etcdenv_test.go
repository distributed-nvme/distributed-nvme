...
func startEtcd(bin string) (string, func(), error) {
	...
	cmd := exec.Command(
		...
		// below the 486 compares the sp drain's maximum-shape D2 batch reaches
		// (§11.6, SPD13; SPD14 is the tripwire pair that keeps it so).
		// DeleteClone's 256-key rectangle sweep was this flag's founding
		// justification and is gone — the clone drain's 68-op batches fit
		// the default (CLD11).
